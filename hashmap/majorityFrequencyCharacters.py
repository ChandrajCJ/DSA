...
class Solution:
    def majorityFrequencyGroup(self, s: str) -> str:
        ...
        max_freq=0
        max_grp_size=0
        final_str_freq_len=0
        fstring=""
        #get frequence map {a: 3, b: 3}
        for i in range(len(s)):
            fmap[s[i]] = fmap.get(s[i] , 0) +1
        print(fmap)
        #get frequency group {4: d, 3: ab}
        for k, value in fmap.items():
            freq_grp[value] = k + freq_grp.get(value, "")
        print(freq_grp)

        for k, value in freq_grp.items():
            if(len(value)>max_grp_size):
                max_grp_size = len(value)
                max_freq = k
                fstring = value
            elif(len(value)==max_grp_size and k>max_freq):
                max_freq=k
                fstring = value
        return fstring
